...
void PrintASN1TestInteger(const char *paramName, const asn1SccTestInteger *pData)
{
    (void)paramName;
    (void)pData;
#ifdef __linux__
...
#endif
#ifdef __unix__
    //printf("%s TestInteger ::= ", paramName);
    printf("%s ", paramName);
    #if WORD_SIZE==8
    printf("%"PRId64, (*pData));
    #else
...
void PrintASN1TestArray(const char *paramName, const asn1SccTestArray *pData)
{
    (void)paramName;
    (void)pData;
#ifdef __linux__
...
void PrintASN1TestStruct(const char *paramName, const asn1SccTestStruct *pData)
{
    (void)paramName;
    (void)pData;
#ifdef __linux__
...
#endif
#ifdef __unix__
    //printf("%s TestStruct ::= ", paramName);
    printf("%s ", paramName);
    printf("{");
    printf("id ");
    #if WORD_SIZE==8
    printf("%"PRId64, (*pData).id);
    #else
    printf("%d", (*pData).id);
    #endif
    printf(", ");
    printf("payload ");
    {
        int i2;
        printf("{");
        for(i2=0; i2<(*pData).payload.nCount; i2++) {
            if (i2) 
                printf(",");
            #if WORD_SIZE==8
            printf("%"PRId64, (*pData).payload.arr[i2]);
            #else
            printf("%d", (*pData).payload.arr[i2]);
            #endif
        }
        printf("}");
    }
...
void PrintASN1PID(const char *paramName, const asn1SccPID *pData)
{
    ...
    switch((*pData)) {
    case 0:
        printf("corecomponent");
        break;
    case 1:
        printf("orchestratorcomponent");
        break;
    case 2:
        printf("utilitycomponent");
        break;
    case 3:
        printf("env");
        break;
